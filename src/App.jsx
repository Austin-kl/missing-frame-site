  ...
  return <header className="sticky top-0 z-40 border-b border-[#E8E1D8]/10 bg-[#0A0A0B]/80 backdrop-blur-xl">
    <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 md:px-6">
      <button type="button" onClick={() => navigate(routes.home)} className="flex items-center gap-3 text-left">
        <img src="/assets/logo/missing-frame-logo.png" alt="Missing Frame" className="h-11 w-auto max-w-[190px] object-contain sm:h-12 sm:max-w-[230px]" />
      </button>
...
const systemProcessImages = [
  ["Паспорт проекта", "Основной документ, демонстрирующий текущее состояние проекта", "/assets/cases/production-system/process/project-passport.webp"],
  ["Shot list", "Кадры, зоны и маршрут", "/assets/cases/production-system/process/shot-list.webp"],
  ["Матрица компетенций", "Структурная система для определения исполнителя для проекта", "/assets/cases/production-system/process/competency-matrix.webp"],
  ["Дорожная карта", "Этапы и сроки выдачи", "/assets/cases/production-system/process/roadmap.webp"],
  ["Архив", "Структурированная база данных материалов проекта", "/assets/cases/production-system/process/archive.webp"],
  ["Итоги", "Разбор проекта по методологии PCA, проведение контрольного среза", "/assets/cases/production-system/process/pca-review.webp"],
];

function SystemImageSlot({ title, text, src }) {
  return <SurfaceCard className="overflow-hidden p-3 hover:border-[#E8E1D8]/22 hover:bg-[#E8E1D8]/[0.06]">
    <ImageFrame className="aspect-[4/3]" src={src} label={title} />
    <div className="p-4">
      <h3 className="text-2xl font-semibold leading-[1.18] tracking-[-0.025em]">{title}</h3>
