...
const cases = [
  {
    ...
    result: "Корпоративный кейс, который показывает работу с командой, таймингом, несколькими зонами и материалами для PR-отчётности.",
    gallery: Array.from({ length: 8 }, (_, i) => `/assets/cases/glowbyte/gallery/${String(i + 1).padStart(2, "0")}.webp`),
  },
  {
    id: "graduation",
    ...
    result: "Доказательство работы с масштабом, участниками, дедлайнами и ответственностью за результат.",
    gallery: Array.from({ length: 8 }, (_, i) => `/assets/cases/mediavypusknoy-2025/gallery/${String(i + 1).padStart(2, "0")}.webp`),
  },
  {
    id: "sports",
    ...
    result: "Переносимая логика для стендов, презентаций, партнёрских зон и активностей.",
    gallery: Array.from({ length: 12 }, (_, i) => `/assets/cases/sports-live-events/gallery/${String(i + 1).padStart(2, "0")}.webp`),
  },
  {
    id: "system",
    ...
    task: "Сделать проект управляемым: от задачи и сценарной логики до финальной выдачи и архива.",
    result: "Клиент понимает, кто отвечает, где лежат материалы, когда будут версии и как использовать контент дальше.",
    gallery: Array.from({ length: 6 }, (_, i) => `/assets/cases/production-system/gallery/${String(i + 1).padStart(2, "0")}.webp`),
  },
  {
    id: "svvfit",
    title: "SVVFIT",
    ...
    task: "Упаковать экспертность тренера и связать короткий контент с коммуникацией и партнёрскими предложениями.",
    result: "Визуальная база для личного бренда, соцсетей и партнёрских интеграций.",
    gallery: Array.from({ length: 6 }, (_, i) => `/assets/cases/svvfit/gallery/${String(i + 1).padStart(2, "0")}.webp`),
  },
  {
    id: "portal",
    ...
    task: "Создать тёмный клиповый мир с читаемой драмой, пластикой и акцентной цветовой схемой.",
    result: "Кейс, который показывает художественный язык и работу с артистическим визуалом.",
    gallery: Array.from({ length: 6 }, (_, i) => `/assets/cases/portal-13/gallery/${String(i + 1).padStart(2, "0")}.webp`),
  },
  {
    id: "shortfilm",
    ...
    gallery: Array.from({ length: 6 }, (_, i) => `/assets/cases/short-film-ai/gallery/${String(i + 1).padStart(2, "0")}.webp`),
  },
];
...
const services = [
  "События и партнёрский контент",
  "Деловой и экспертный контент",
  "Реклама и бренд-видео",
  "Клипы и визуальная упаковка",
];

const processSteps = ["Стратегия", "Креатив", "Производство", "Выдача", "Аналитика"];
...
function PageShell({ children }) {
  return (
    <div className="min-h-screen bg-[#0A0A0B] text-[#E8E1D8] selection:bg-transparent">
...
function Nav({ route, navigate }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[#E8E1D8]/10 bg-[#0A0A0B]/80 backdrop-blur-xl">
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[999] lg:hidden">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.97, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98, y: 12 }} className="relative flex h-[100dvh] flex-col overflow-y-auto overscroll-contain bg-[#020203]/95 px-6 py-6 text-center" onTouchMove={(event) => event.stopPropagation()}>
...
function VisualBox({ label, src, alt, className = "", contain = false }) {
  const [failed, setFailed] = useState(false);
  return (
    <div className={cls("relative overflow-hidden rounded-[1.6rem] border border-[#E8E1D8]/10 bg-[#111]", className)}>
      {src && !failed ? <img src={src} alt={alt || label} onError={() => setFailed(true)} className={cls("h-full w-full opacity-85", contain ? "object-contain object-bottom" : "object-cover")} /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(143,31,35,0.45),transparent_32%),linear-gradient(135deg,rgba(232,225,216,0.12),rgba(52,15,18,0.42)_48%,rgba(10,10,11,1))]" />}
      <div className="absolute bottom-6 left-6 text-xs uppercase tracking-[0.22em] text-[#E8E1D8]/50">{label}</div>
    </div>
  );
}
  ...
  return (
    <div className="overflow-hidden rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-3">
      <div className="relative aspect-video overflow-hidden rounded-[1.45rem] bg-[#0A0A0B]">
        {!failed ? <video controls poster={poster} className="h-full w-full object-cover" onError={() => setFailed(true)}><source src={src} type="video/mp4" /></video> : <VisualBox label={label} className="h-full" />}
      </div>
    </div>
  );
}
...
function PartnersStrip() {
  return <section className="mx-auto max-w-7xl px-4 py-8 md:px-6"><div className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-5"><Eyebrow>партнёры и проекты</Eyebrow><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{partners.map((partner) => <div key={partner} className="flex min-h-20 items-center justify-center rounded-2xl border border-[#E8E1D8]/10 bg-[#0A0A0B]/35 px-4 text-center text-sm font-semibold text-[#E8E1D8]/70">{partner}</div>)}</div></div></section>;
}
...
function CasesCarousel({ navigate }) {
  ...
  return (
    <section className="mx-auto max-w-7xl select-none overflow-hidden px-4 py-10 md:px-6 md:py-14">
      ...
      <div className="md:hidden"><div className="overflow-hidden rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-3"><VisualBox label={`${active.short}: обложка`} src={active.loop || active.cover} alt={active.title} className="aspect-[5/6]"/><div className="p-4"><div className="mb-3 flex items-center justify-between gap-3"><span className="rounded-full border border-[#E8E1D8]/12 px-3 py-1 text-xs text-[#E8E1D8]/60">{String(index + 1).padStart(2, "0")} / {String(total).padStart(2, "0")}</span><span className="rounded-full border border-[#E8E1D8]/12 px-3 py-1 text-[10px] uppercase tracking-[0.15em] text-[#E8E1D8]/55">{active.type}</span></div><h3 className="text-3xl font-semibold leading-[0.98] tracking-[-0.04em]">{active.title}</h3><p className="mt-4 text-sm leading-6 text-[#E8E1D8]/62">{active.lead}</p></div></div></div>
      <div className="relative mx-auto hidden h-[500px] max-w-6xl md:block"><div className="pointer-events-none absolute inset-y-0 left-0 z-20 w-44 bg-gradient-to-r from-[#0A0A0B] to-transparent"/><div className="pointer-events-none absolute inset-y-0 right-0 z-20 w-44 bg-gradient-to-l from-[#0A0A0B] to-transparent"/>{cases.map((item, i) => { const off = offsetFor(i); const abs = Math.abs(off); const is = off === 0; return <motion.div key={item.id} className="absolute left-1/2 top-0 h-full w-[72%] max-w-[560px] -translate-x-1/2 overflow-hidden rounded-[2.2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-3 shadow-2xl" animate={{ x: off * 205, scale: is ? 0.92 : abs === 1 ? 0.76 : 0.62, opacity: is ? 1 : abs === 1 ? 0.42 : abs === 2 ? 0.14 : 0, filter: is ? "blur(0px)" : abs === 1 ? "blur(5px)" : "blur(12px)", zIndex: 20 - abs }} transition={{ type: "spring", stiffness: 120, damping: 22 }} style={{ pointerEvents: "none" }}><VisualBox label={`${item.short}: обложка`} src={item.loop || item.cover} alt={item.title} className="h-[68%]"/><div className="absolute left-8 top-8 rounded-full border border-[#E8E1D8]/15 bg-[#0A0A0B]/55 px-3 py-1 text-xs">{String(i + 1).padStart(2, "0")} / {String(total).padStart(2, "0")}</div><div className="absolute bottom-8 left-8 right-8"><div className="mb-3 inline-flex rounded-full border border-[#E8E1D8]/15 bg-[#0A0A0B]/55 px-3 py-1 text-xs uppercase tracking-[0.18em]">{item.type}</div><h3 className="text-4xl font-semibold tracking-[-0.04em]">{item.title}</h3><p className="mt-3 text-sm leading-6 text-[#E8E1D8]/65">{item.lead}</p></div></motion.div>; })}</div>
      <div className="mt-6 flex items-center justify-center gap-3"><button type="button" onClick={prev} className="rounded-full border border-[#E8E1D8]/15 p-4"><ChevronLeft className="h-5 w-5"/></button><button type="button" onClick={() => navigate(`/cases/${active.id}`)} className="rounded-full bg-[#E8E1D8] px-5 py-4 text-sm font-semibold text-[#0A0A0B]">Открыть кейс →</button><button type="button" onClick={next} className="rounded-full border border-[#E8E1D8]/15 p-4"><ChevronRight className="h-5 w-5"/></button></div>
    </section>
  );
}
...
function TeamCarousel() {
  ...
  return (
    <section className="mx-auto max-w-7xl select-none overflow-hidden px-4 py-10 md:px-6 md:py-14">
      <div className="mb-7"><Eyebrow>команда</Eyebrow><div className="mb-3 flex gap-3 text-sm text-[#E8E1D8]/50"><span className="rounded-full border border-[#E8E1D8]/12 px-3 py-1">{String(index + 1).padStart(2, "0")} / {String(total).padStart(2, "0")}</span><span>{active.role}</span></div><h2 className="text-3xl font-semibold tracking-[-0.045em] md:text-5xl">Команда Missing Frame</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[#E8E1D8]/58">Управление, техника, коммерция и креатив в одной системе.</p></div>
      <div className="md:hidden"><div className="mx-auto max-w-[340px]"><TeamCard member={active} index={index} compact/></div></div>
      <div className="relative mx-auto hidden h-[600px] max-w-6xl md:block"><div className="pointer-events-none absolute inset-y-0 left-0 z-20 w-44 bg-gradient-to-r from-[#0A0A0B] to-transparent"/><div className="pointer-events-none absolute inset-y-0 right-0 z-20 w-44 bg-gradient-to-l from-[#0A0A0B] to-transparent"/>{team.map((member, i) => { const off = offsetFor(i); const abs = Math.abs(off); const is = off === 0; return <motion.div key={member.id} className="absolute left-1/2 top-0 h-full w-[72%] max-w-[380px] -translate-x-1/2" animate={{ x: off * 205, scale: is ? 0.92 : abs === 1 ? 0.76 : 0.62, opacity: is ? 1 : abs === 1 ? 0.42 : abs === 2 ? 0.14 : 0, filter: is ? "blur(0px)" : abs === 1 ? "blur(5px)" : "blur(12px)", zIndex: 20 - abs }} transition={{ type: "spring", stiffness: 120, damping: 22 }} style={{ pointerEvents: is ? "auto" : "none" }}><TeamCard member={member} index={i} compact/></motion.div>; })}</div>
      <div className="mt-6 flex items-center justify-center gap-3"><button type="button" onClick={prev} className="rounded-full border border-[#E8E1D8]/15 p-4"><ChevronLeft className="h-5 w-5"/></button><div className="rounded-full border border-[#E8E1D8]/12 px-5 py-4 text-sm text-[#E8E1D8]/70">{active.name}</div><button type="button" onClick={next} className="rounded-full border border-[#E8E1D8]/15 p-4"><ChevronRight className="h-5 w-5"/></button></div>
    </section>
  );
}

function ServicesTeaser({ navigate }) {
  return <section className="mx-auto max-w-7xl px-4 py-12 md:px-6 md:py-16"><div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><Eyebrow>услуги</Eyebrow><h2 className="text-3xl font-semibold tracking-[-0.045em] md:text-5xl">Что можем закрыть</h2><p className="mt-4 max-w-2xl text-sm leading-6 text-[#E8E1D8]/60">Отдельная съёмка, комплексный продакшен, контент-пакет для события или регулярная визуальная поддержка бренда.</p></div><button type="button" onClick={() => navigate(routes.services)} className="w-fit rounded-full bg-[#E8E1D8] px-5 py-3 font-semibold text-[#0A0A0B]">Весь спектр услуг →</button></div><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">{services.map((service) => <div key={service} className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-5"><div className="mb-5 h-11 w-11 rounded-2xl border border-[#E8E1D8]/12 bg-[#8F1F23]/12"/><h3 className="text-xl font-semibold tracking-[-0.035em]">{service}</h3><p className="mt-3 text-sm leading-6 text-[#E8E1D8]/58">Подбираем формат под задачу, площадку, сроки и нужные материалы.</p></div>)}</div></section>;
}
...
function ServicesPage({ navigate }) {
  return <main className="mx-auto max-w-7xl px-4 py-14 md:px-6 md:py-20"><SectionTitle eyebrow="услуги" title="Услуги Missing Frame" text="Собираем контент под задачу: от идеи и съёмки до монтажа, адаптаций и выдачи."/><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">{services.map((service) => <div key={service} className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-6"><h3 className="text-2xl font-semibold tracking-[-0.04em]">{service}</h3><p className="mt-4 text-sm leading-6 text-[#E8E1D8]/62">Формат, команда и выдача подбираются под проект.</p></div>)}</div><ProjectStartBlock navigate={navigate}/></main>;
}

function CasesPage({ navigate }) {
  return <main className="mx-auto max-w-7xl px-4 py-14 md:px-6 md:py-20"><SectionTitle eyebrow="портфолио" title="Кейсы Missing Frame" text="События, бренды, клипы и система работы."/><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{cases.map((item, i) => <button key={item.id} type="button" onClick={() => navigate(`/cases/${item.id}`)} className="overflow-hidden rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-3 text-left"><VisualBox label={`${item.short}: обложка`} src={item.loop || item.cover} alt={item.title} className="aspect-[16/10]"/><div className="p-4"><div className="mb-3 flex justify-between text-xs text-[#E8E1D8]/45"><span>{item.type}</span><span>{String(i + 1).padStart(2, "0")}</span></div><h3 className="text-2xl font-semibold tracking-[-0.04em]">{item.title}</h3><p className="mt-3 text-sm leading-6 text-[#E8E1D8]/58">{item.lead}</p></div></button>)}</div></main>;
}

function ProcessPage() {
  return <main className="mx-auto max-w-7xl px-4 py-14 md:px-6 md:py-20"><SectionTitle eyebrow="система работы" title="Понятный производственный процесс" text="Фиксируем задачу, роли, сроки, съёмочный план и выдачу материалов."/><div className="grid gap-4 lg:grid-cols-5">{processSteps.map((step, i) => <div key={step} className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-6"><div className="mb-10 text-xs uppercase tracking-[0.22em] text-[#8F1F23]">0{i + 1}</div><h2 className="text-2xl font-semibold">{step}</h2><p className="mt-4 text-sm leading-6 text-[#E8E1D8]/62">Этап фиксируется заранее и проходит через контроль результата.</p></div>)}</div></main>;
}
...
function ContactPage() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-14 md:px-6 md:py-20"><div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]"><div><Eyebrow>контакты</Eyebrow><h1 className="text-4xl font-semibold tracking-[-0.055em] md:text-7xl">Запустить проект или обсудить пакет за 15–20 минут.</h1><p className="mt-6 text-lg leading-8 text-[#E8E1D8]/66">Основной контакт: Екатерина / Missing Frame production.</p><a href="mailto:partner@missingframe.ru" className="mt-8 inline-flex rounded-full border border-[#E8E1D8]/15 px-5 py-3 text-[#E8E1D8]/75">partner@missingframe.ru</a></div><form onSubmit={submit} className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-5 md:p-8"><div className="grid gap-4"><Input name="projectName" label="Название проекта" required/><Textarea name="description" label="Описание задачи" required/><Input name="email" label="Email" type="email" required/><Input name="contact" label="Telegram / VK / телефон"/><button type="submit" disabled={sending} className="rounded-full bg-[#8F1F23] px-5 py-3 font-semibold disabled:opacity-60">{sending ? "Отправляем..." : "Отправить заявку"}</button>{sent ? <div className="rounded-2xl border border-[#8F1F23]/35 bg-[#8F1F23]/12 p-4 text-sm">Спасибо. Заявка принята.</div> : null}{error ? <div className="rounded-2xl border border-[#E8E1D8]/15 bg-[#0A0A0B]/35 p-4 text-sm">{error}</div> : null}</div></form></div></main>;
}

function CaseDetailPage({ item, navigate }) {
  if (!item) return <NotFound navigate={navigate}/>;
  return <main><section className="mx-auto max-w-7xl px-4 py-10 md:px-6 md:py-16"><button type="button" onClick={() => navigate(routes.cases)} className="mb-8 rounded-full border border-[#E8E1D8]/12 px-4 py-2 text-sm">← Все кейсы</button><div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]"><div><Eyebrow>{item.type}</Eyebrow><h1 className="text-4xl font-semibold tracking-[-0.055em] md:text-7xl">{item.title}</h1><p className="mt-6 text-lg leading-8 text-[#E8E1D8]/66">{item.lead}</p><div className="mt-8 grid gap-4 md:grid-cols-2"><div className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-6"><h2 className="text-2xl font-semibold">Задача</h2><p className="mt-4 text-sm leading-7 text-[#E8E1D8]/62">{item.task}</p></div><div className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-6"><h2 className="text-2xl font-semibold">Результат</h2><p className="mt-4 text-sm leading-7 text-[#E8E1D8]/62">{item.result}</p></div></div></div><VisualBox label={`${item.short}: обложка`} src={item.cover} alt={item.title} className="aspect-[16/10]"/></div></section><section className="mx-auto max-w-7xl px-4 py-8 md:px-6"><SectionTitle eyebrow="материалы" title="Фото и видео" text="Основной ролик и галерея кейса."/><div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]"><VideoBlock src={item.video} poster={item.cover} label="основное видео"/><div className="grid grid-cols-2 gap-3">{item.gallery.slice(0, 6).map((src, i) => <VisualBox key={src} src={src} label={`кадр ${String(i + 1).padStart(2, "0")}`} className="aspect-square" />)}</div></div></section><ProjectStartBlock navigate={navigate}/></main>;
}

function ProjectStartBlock({ navigate }) {
  return <section className="mx-auto max-w-7xl px-4 py-12 md:px-6 md:py-16"><div className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/5 p-6 md:p-10"><Eyebrow>создать проект</Eyebrow><h2 className="max-w-3xl text-3xl font-semibold tracking-[-0.045em] md:text-5xl">Соберём формат под вашу задачу.</h2><p className="mt-5 max-w-2xl leading-7 text-[#E8E1D8]/62">Опишите проект, сроки и нужные материалы — предложим рабочий формат съёмки и выдачи.</p><button type="button" onClick={() => navigate(routes.contact)} className="mt-8 rounded-full bg-[#E8E1D8] px-5 py-3 font-semibold text-[#0A0A0B]">Создать свой проект →</button></div></section>;
}
...
export default function App() {
  ...
  const page = useMemo(() => {
    ...
    if (route.startsWith("/cases/")) return <CaseDetailPage item={cases.find((item) => item.id === route.split("/").pop())} navigate={navigate}/>;
