...
function SystemWorkBlock({ number, title, text, imageTitle, imageText }) {
  return <div className="grid gap-5 rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-5 transition hover:border-[#8F1F23]/45 hover:bg-[#340F12]/35 lg:grid-cols-[0.88fr_1.12fr] lg:items-center">
    <div>
      <div className="mb-8 text-xs uppercase tracking-[0.28em] text-[#8F1F23]">{number}</div>
      <h3 className="text-3xl font-semibold leading-tight tracking-[-0.04em]">{title}</h3>
      <p className="mt-4 text-sm leading-7 text-[#E8E1D8]/64">{text}</p>
    </div>
    <SystemImageSlot title={imageTitle} text={imageText} />
  </div>;
}

function ProductionSystemCasePage({ item, navigate }) {
  const systemBlocks = [
    ["Production-паспорт", "Фиксируем цель проекта, формат, аудиторию, площадки публикации, ключевые сообщения, дедлайны, ответственных и набор материалов на выдачу."],
    ["Shot list и маршруты", "Разбиваем проект на обязательные кадры, зоны, сцены, спикеров, активности и технические точки контроля."],
    ["Командная матрица", "Заранее распределяем роли: кто отвечает за креатив, технику, коммуникацию, съёмку, монтаж, выдачу и финальную проверку."],
    ["График выдачи", "Разделяем быстрые превью, short-form, фотоотчёт, основной ролик, архив и финальные версии под площадки."],
    ["Архив и структура", "Собираем исходники, финальные версии, материалы для соцсетей, пост-релизов и повторного использования в понятную систему папок."],
    ["Разбор результата", "Фиксируем, что сработало, какие материалы можно переиспользовать и как масштабировать следующий проект."],
  ];

  return <main>
    <section className="mx-auto max-w-7xl px-4 py-10 md:px-6 md:py-16">
      <button onClick={() => navigate(routes.cases)} className="mb-8 rounded-full border border-[#E8E1D8]/12 px-4 py-2 text-sm">← Все кейсы</button>
      <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-end">
        <div>
          <Eyebrow>{item.type}</Eyebrow>
          <h1 className="text-4xl font-semibold leading-[1.08] tracking-[-0.055em] md:text-7xl">{item.title}</h1>
          <p className="mt-6 text-lg leading-8 text-[#E8E1D8]/66">{item.lead}</p>
        </div>
        <VisualBox className="aspect-[16/10]" />
      </div>
      <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-4 text-sm text-[#E8E1D8]/66">Формат: production-система</div>
        <div className="rounded-2xl border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-4 text-sm text-[#E8E1D8]/66">Роль: структура и контроль</div>
        <div className="rounded-2xl border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-4 text-sm text-[#E8E1D8]/66">Материалы: документы, схемы, тайминг</div>
        <div className="rounded-2xl border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-4 text-sm text-[#E8E1D8]/66">Результат: управляемый проект</div>
      </div>
    </section>

    <section className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12">
      <SectionTitle eyebrow="как работаем" title="Из чего состоит production-система" text="В этом кейсе показываем подход к сборке проекта: от брифа до выдачи материалов и разбора результата" />
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {systemBlocks.map(([title, text]) => <div key={title} className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-6 transition hover:border-[#8F1F23]/45 hover:bg-[#340F12]/35"><h3 className="text-2xl font-semibold leading-tight tracking-[-0.035em]">{title}</h3><p className="mt-4 text-sm leading-7 text-[#E8E1D8]/62">{text}</p></div>)}
      </div>
    </section>

    <section className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12">
      <div className="grid gap-5 lg:grid-cols-[1fr_1fr]">
        <div className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-6 md:p-8">
          <Eyebrow>что получает клиент</Eyebrow>
          <h2 className="text-3xl font-semibold leading-tight tracking-[-0.04em]">Проект становится понятным до съёмки</h2>
          <p className="mt-4 text-sm leading-7 text-[#E8E1D8]/62">Клиент заранее видит состав команды, список материалов, логику съёмки, сроки выдачи и структуру архива. Это снижает хаос на площадке и ускоряет согласование результата.</p>
        </div>
        <div className="rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-6 md:p-8">
          <Eyebrow>как масштабируется</Eyebrow>
          <h2 className="text-3xl font-semibold leading-tight tracking-[-0.04em]">Один проект раскладывается на несколько форматов</h2>
          <p className="mt-4 text-sm leading-7 text-[#E8E1D8]/62">Aftermovie, фотоотчёт, vertical clips, backstage, интервью, обложки, посты и архив исходников собираются в одну систему, а не создаются хаотично после съёмки.</p>
        </div>
      </div>
    </section>
...
function CaseDetailPage({ item, navigate }) {
  ...
  if (item.id === "system" || item.id === "production-system") return <ProductionSystemCasePage item={item} navigate={navigate} />;
  return <main><section className="mx-auto max-w-7xl px-4 py-10 md:px-6 md:py-16"><button onClick={() => navigate(routes.cases)} className="mb-8 rounded-full border border-[#E8E1D8]/12 px-4 py-2 text-sm">← Все кейсы</button><div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]"><div><Eyebrow>{item.type}</Eyebrow><h1 className="text-4xl font-semibold tracking-[-0.055em] md:text-7xl">{item.title}</h1><p className="mt-6 text-lg leading-8 text-[#E8E1D8]/66">{item.lead}</p><div className="mt-6 rounded-[2rem] border border-[#E8E1D8]/10 bg-[#E8E1D8]/[0.04] p-5"><h2 className="text-2xl font-semibold tracking-[-0.04em]">Задача</h2><p className="mt-3 text-sm leading-7 text-[#E8E1D8]/62">{item.task || "Задача кейса будет дополнена после загрузки материалов."}</p></div></div><VisualBox className="aspect-[16/10]"/></div></section><section className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12"><SectionTitle eyebrow="материалы" title="Основное видео" text="Здесь будет размещён главный ролик кейса. Рамка зафиксирована по формату видео и не уезжает ниже окна."/><VideoFrame title="основное видео" /></section><section className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12"><SectionTitle eyebrow="фото" title="Галерея кейса"/><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><VisualBox className="aspect-[4/3]"/><VisualBox className="aspect-[4/3]"/><VisualBox className="aspect-[4/3]"/><VisualBox className="aspect-[4/3]"/><VisualBox className="aspect-[4/3]"/><VisualBox className="aspect-[4/3]"/></div></section><ProjectStartBlock navigate={navigate}/></main>;
}
